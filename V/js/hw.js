(function() {
	...
	Dldh.Drag = function (el, rowCls, columnCls) {
	  var rs = {};
	  rs.rowCls = rowCls;
	  rs.columnCls = columnCls;
	  rs.wrap = HW.getDom(el);
	  ...
	  rs.down = function(x, y) {
	    if(HW.Css.hasClass(this.target, this.rowCls) || HW.Css.hasClass(this.target, this.columnCls)) {
	      ...
	      this.isMove = true;
	      if(HW.Css.hasClass(this.target, this.rowCls)) {
	        this.cls = this.rowCls;   
	      }
	      if(HW.Css.hasClass(this.target, this.columnCls)) {
	        this.cls = this.columnCls;
	      }
	      HW.Event.bind(document, "mousemove", rs._move);
	      HW.Event.bind(document, "mouseup", rs._up);
	    }  
	  }
	  ...
	  rs.up = function(x, y) {
	    if(this.current == this.target) {
	      ...
	      if(this.isMove) {
	        ...
	        if(this.isMove && this.check) {
	          ...
	          var xy = HW.Css.getXY(this.target);
	          this.$proxy.animate({
	            left : xy[0],
	            top : xy[1]
	          }, 0.35 * 1000, undefined, () => {
	    ...
	    this.target = null;
	    this.current = null;
	    HW.Event.unbind(document, "mousemove", rs._move);
	    HW.Event.unbind(document, "mouseup", rs._up);
	  }
	  HW.Event.bind(rs.wrap, "mousedown", rs._down);
	  return rs;
	}
	window.HW = Dldh;
})();